...
<body>
  ...
  <button id="toggle-menu">☰</button>
    ...
    <div class="list-group">
      ...
      <?php
        ...
        $query_categorias = "SELECT id_cat_sticker, nombre_cat_sticker FROM categorias_sticker";
        $result_categorias = mysqli_query($conexion, $query_categorias);

        while ($categoria = mysqli_fetch_assoc($result_categorias)) {
            $enlace_categoria = "tienda_sticker.php?categoria=" . $categoria['id_cat_sticker'];
            // Mantener el parámetro is_3d
            $enlace_categoria .= ($is_3d !== '') ? "&is_3d=$is_3d" : '';
            echo '<a href="' . $enlace_categoria . '" class="list-group-item" id="main-category">' . $categoria['nombre_cat_sticker'] . '</a>';

            $query_subcategorias = "SELECT id_subcat_sticker, nombre_subcat_sticker FROM subcategorias_sticker WHERE id_cat_sticker=" . $categoria['id_cat_sticker'];
            $result_subcategorias = mysqli_query($conexion, $query_subcategorias);

            while ($subcategoria = mysqli_fetch_assoc($result_subcategorias)) {
                $enlace_subcategoria = "tienda_sticker.php?categoria=" . $categoria['id_cat_sticker'] . '&subcategoria=' . $subcategoria['id_subcat_sticker'];
                // Mantener el parámetro is_3d
                $enlace_subcategoria .= ($is_3d !== '') ? "&is_3d=$is_3d" : '';
                echo '<a href="' . $enlace_subcategoria . '" class="list-group-item" id="sub-category">' . $subcategoria['nombre_subcat_sticker'] . '</a>';
            }
        }
      ?>
    </div>
      ...
      <ul class="productos-list">
          <?php
            ...
            foreach ($productosMostrar as $producto) {
              ...
              echo "<a href='producto.php?id=" . $producto['id'] . "'><div class='card-button'> Ver más";
              echo "</div></a>";
              echo "</div></div>";
            }
          ?>
  ...
  <div class="pagination">
    <?php
      ...
      $enlace_anterior = ($pagina_anterior > 1) ? "tienda_sticker.php?pagina=$pagina_anterior&is_3d=$is_3d" : "tienda_sticker.php?pagina=1&is_3d=$is_3d";
      echo "<a href='$enlace_anterior'>Anterior</a>";

      // Construir los enlaces para las páginas intermedias
      for ($i = 1; $i <= $total_paginas; $i++) {
          $enlace_pagina = "tienda_sticker.php?pagina=$i&is_3d=$is_3d";
          if ($i == $pagina_actual) {
      ...
      $enlace_siguiente = ($pagina_siguiente <= $total_paginas) ? "tienda_sticker.php?pagina=$pagina_siguiente&is_3d=$is_3d" : "tienda_sticker.php?pagina=$total_paginas&is_3d=$is_3d";
